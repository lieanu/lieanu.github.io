...
没写过Android代码，也没搞过这一块的反调试，打开脑洞猜吧。写了几个对应的解密的函数，对一堆可疑的字符串解密后，是一些诸如`dlsym/getpid`之类的libc函数。这不应该是反调试的地方啊，没有对应的敏感函数。简单故意改错几个字符串，然后再重打包。
...
感谢阿里移动安全挑战赛，让本来对移动安全不感冒的我，也生起了极大的兴趣。
